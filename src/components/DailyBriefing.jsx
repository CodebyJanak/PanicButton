          ...
          <div className="px-6 py-5">
            {!data && !loading && (
              <div className="text-center py-8">
                ...
                <p style={{ fontFamily:'var(--font-ui)', fontWeight:600, fontSize:'18px', color:'var(--text-1)', marginBottom:'8px' }}>
                  Good {getGreeting()}
                </p>
                {!hasArticles ? (
                  <div className="mb-6 px-4 py-3 rounded-xl" style={{ background:'rgba(255,179,71,0.08)', border:'1px solid rgba(255,179,71,0.2)' }}>
                    <p style={{ fontFamily:'var(--font-mono)', fontSize:'10px', color:'var(--amber)', letterSpacing:'0.05em', lineHeight:1.8 }}>
                      ⚠️ News is still loading. Please wait a moment and try again.
                    </p>
                  </div>
                ) : (
                  <p className="mb-6" style={{ fontFamily:'var(--font-body)', fontSize:'13px', color:'var(--text-2)', lineHeight:1.7 }}>
                    Claude will read today's top {articles.length} headlines and generate a personalized briefing.
                  </p>
                )}
                {error && (
                  <div className="mb-4 px-4 py-3 rounded-xl" style={{ background:'rgba(255,0,0,0.08)', border:'1px solid rgba(255,0,0,0.2)' }}>
                    <p style={{ fontFamily:'var(--font-mono)', fontSize:'10px', color:'var(--red)', lineHeight:1.7 }}>⚠️ {error}</p>
                  </div>
                )}
                <button onClick={generate} disabled={!hasArticles || loading}
                  className="btn-panic px-8 py-3 rounded-xl disabled:opacity-40"
            ...
            {loading && (
              <div className="flex flex-col items-center py-10 gap-4">
                ...
                <p style={{ fontFamily:'var(--font-mono)', fontSize:'10px', color:'var(--text-3)', letterSpacing:'0.1em' }}>
                  Claude is reading today's news…
                </p>
              </div>
            )}
            ...
            {data && !loading && (
              <div className="flex flex-col gap-5">
                ...
                <div className="p-4 rounded-xl" style={{ background:'rgba(255,179,71,0.06)', border:'1px solid rgba(255,179,71,0.15)' }}>
                  ...
                  <p style={{ fontFamily:'var(--font-display)', fontSize:'clamp(16px,3vw,22px)', letterSpacing:'0.04em', color:'var(--text-1)', lineHeight:1.2 }}>
                    {data.headline}
                  </p>
                </div>
                ...
                {data.closingNote && (
                  <div className="p-4 rounded-xl text-center" style={{ background:'rgba(0,245,255,0.04)', border:'1px solid rgba(0,245,255,0.1)' }}>
                    <p style={{ fontFamily:'var(--font-body)', fontSize:'13px', color:'var(--cyan)', lineHeight:1.6, fontStyle:'italic' }}>
                      "{data.closingNote}"
                    </p>
                  </div>
                )}

                <button onClick={() => { setData(null); setError(null); }} className="btn-ghost px-4 py-2 rounded-lg text-center cursor-pointer"
