// All Claude API calls go through /api/ai/messages (our Express proxy)
// because the browser cannot call api.anthropic.com directly (CORS blocked)
...
export async function summarizeArticle(article) {
  const system = 'You are a concise news summarizer. Respond ONLY with valid JSON, no markdown: {"bullets":["point 1.","point 2.","point 3."],"readTime":2,"keyFact":"The most important single fact."}';
...
export async function analyzeSentiment(headlines) {
  const system = 'You are a sentiment analyzer. Respond ONLY with valid JSON, no markdown: {"positive":25,"negative":50,"neutral":25,"score":-0.3,"dominantTheme":"geopolitical tension","mood":"anxious"}';
...
export async function extractTrendingTopics(articles) {
  const system = 'Topic extractor. Respond ONLY with valid JSON array, no markdown: [{"word":"topic","count":15,"category":"politics"}]';
...
export async function detectBias(article) {
  const system = 'Media bias analyzer. Respond ONLY with valid JSON, no markdown: {"bias":"left","confidence":0.8,"reasoning":"Brief explanation.","political_leaning":0.3} where bias is "left"|"center"|"right"|"unknown" and political_leaning is -1 to +1.';
  const user   = `Analyze bias:\nTitle: ${article.title}\nSource: ${article.source?.name || 'Unknown'}\nDescription: ${article.description || ''}\nContent: ${(article.content || '').substring(0, 400)}`;
...
export async function checkFakeNews(article) {
  const system = 'Fact-checker. Respond ONLY with valid JSON, no markdown: {"credibility":"high","score":0.8,"flags":["flag1"],"verdict":"Brief verdict."} where credibility is "high"|"medium"|"low"|"suspicious".';
  const user   = `Fact-check:\nTitle: ${article.title}\nSource: ${article.source?.name || 'Unknown'}\nDescription: ${article.description || ''}\nContent: ${(article.content || '').substring(0, 400)}`;
...
export async function chatWithNews(userMessage, articles, history = []) {
  const system = `You are a smart news assistant. Answer questions about current events using today's headlines.\n\nCurrent articles:\n${articles.slice(0, 20).map(a => `• ${a.title} (${a.source?.name || 'Unknown'})`).join('\n')}`;
  const messages = [
...
export async function generateDailyBriefing(allArticles) {
  ...
  const system = 'Professional news anchor. Respond ONLY with valid JSON, no markdown: {"headline":"Main story in 1 sentence.","sections":[{"category":"Politics","summary":"2-3 sentence summary."}],"closingNote":"Upbeat closing remark."}';
  const grouped = {};
  ...
  const user = 'Generate a daily briefing from:\n' + Object.entries(grouped).map(([cat, titles]) => `${cat}: ${titles.join(' | ')}`).join('\n');
  const raw  = await callClaude(system, user, 1000);
  try { return JSON.parse(raw.replace(/```json|```/g, '').trim()); }
  catch { throw new Error('Failed to parse briefing. Please try again.'); }
}

export async function translateArticle(article, targetLang = 'English') {
  const system = `Translator to ${targetLang}. Respond ONLY with valid JSON, no markdown: {"title":"translated","description":"translated","translatedFrom":"detected language"}`;
  const user   = `Title: ${article.title}\nDescription: ${article.description || ''}`;
